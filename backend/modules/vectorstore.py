...
def add_document_chunks(doc_id: str, filename: str, chunks: list[dict]) -> int:
    ...
    Processes in batches of 50 so the server stays responsive during large
    uploads (e.g. 800-page textbooks that produce 800+ chunks).
    """
    if not chunks:
        return 0
    ...
    for batch_start in range(0, len(chunks), BATCH_SIZE):
        ...
        ids = [
            f"{doc_id}_{batch_start + i}_{uuid.uuid4().hex[:6]}"
